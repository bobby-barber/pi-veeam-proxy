...
- **A full-tunnel VPN on the Pi silently breaks the proxy path.** With WireGuard `AllowedIPs =
  0.0.0.0/0` on the Pi, replies to a proxy on another subnet go out the tunnel and every hook
  fails. Keep the home network in the main table with a `PostUp` route (`10.10.0.x/16 via <LAN
  gateway>`), guarded so it is only added when the gateway is a home address; wg-quick's
  `suppress_prefixlength 0` rule lets it win over the tunnel.
