...
## WireGuard on the Pis vs. the proxy path (2026-09-09, both Pis)

```bash
# diagnose (on the Pi): the proxy's subnet resolved to the tunnel
ip rule                      # 32764: from all lookup main suppress_prefixlength 0 ; 32765: not from all fwmark 0xca6c lookup 51820
ip route get 10.10.0.20    # -> dev BarberHOME table 51820   (broken)

# fix: /etc/wireguard/BarberHOME.conf, [Interface] section
PostUp = r=$(ip -4 route show default | awk '$3 ~ /^10\.100\./ {print $3" dev "$5; exit}'); [ -n "$r" ] && ip route replace 10.10.0.x/16 via $r || true
PreDown = ip route del 10.10.0.x/16 2>/dev/null || true

sudo wg-quick down BarberHOME; sudo wg-quick up BarberHOME     # run over the LAN address, not the tunnel
ip route get 10.10.0.20    # -> via 10.10.0.x dev eth0      (fixed)

# verify from pproxy, then dry-run the per-policy hooks
for ip in 10.10.0.101 10.10.0.102; do ping -c1 -W2 $ip; sudo iscsiadm -m discovery -t sendtargets -p $ip; done
sudo /usr/local/sbin/pi-veeam-pre-pi-02.sh && sudo /usr/local/sbin/pi-veeam-post-pi-02.sh
sudo /usr/local/sbin/pi-veeam-pre-pi-01.sh && sudo /usr/local/sbin/pi-veeam-post-pi-01.sh

# watch a run from the proxy
sudo veeamconfig session list | tail -3
sudo veeamconfig session log --id <session-id> | tail -15
```
